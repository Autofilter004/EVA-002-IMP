...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"<b>🎬 𝖳𝗂𝗍𝗅𝖾 : <a href={imdb['url']}>{imdb.get('title')}</a></b>\n<b>📆 Year : <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</b></a>\n<b>🎭 Genres : {imdb.get('genres')}</b>\n<b>🌟 Rating : <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> / 10</b>\n<b>🕊 Request By : {update.from_user.mention}</b>", reply_markup=InlineKeyboardMarkup(btn))

        elif imdb:

            await message.reply_text(f"<b>🎬 𝖳𝗂𝗍𝗅𝖾 : <a href={imdb['url']}>{imdb.get('title')}</a></b>\n<b>📆 Year : <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a></b>\n<b>🎭 Genres : {imdb.get('genres')}</b>\n<b>🌟 Rating : <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> / 10</b>\n<b>🕊 Request By : {update.from_user.mention}</b>", reply_markup=InlineKeyboardMarkup(btn))
